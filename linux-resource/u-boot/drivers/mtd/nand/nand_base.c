...
static void nand_write_buf16(struct mtd_info *mtd, const uint8_t *buf, int len)
{
	...
	//printf("%s:i am here\n",__func__);
	//printf("%s:p=0x%x,s=0x%x\n",__func__, p, s);
#if 1
	for (i = 0; i < len; i++){
		q= (((*(s+i*2))) &0xff) |(((*(s+i*2+1) )<<8) & 0xff00);
//		printf("p[i]=%d, q=%d\n",p[i],q);
		writew(q, chip->IO_ADDR_W);
...
int nand_scan_tail(struct mtd_info *mtd)
{
	...
	case NAND_ECC_SOFT_BCH:
		 ...
		 * select how many bits are correctable; see nand_bch_init() for details.		 
