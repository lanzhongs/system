...
/*<zhashuang add for led txrx begin*/
#ifdef CONFIG_CHAOYUAN_BOARD
	davinci_eth_phy_read(active_phy_addr,PHY_LCR_2, &tmp);
	tmp &= 0x0fff;
	tmp |= LINK_ESTABLISHED_AND_ACTIVITY << 12; //LEDTXRX activity
	davinci_eth_phy_write(active_phy_addr,PHY_LCR_2, tmp);
#endif
/*zhashuang add for led txrx end>*/
