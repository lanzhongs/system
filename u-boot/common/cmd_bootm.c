...
extern u_int g_boot_part;
int final_bootm (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int ret = 0;
	
	ret = do_bootm(cmdtp, flag, argc, argv);
	//	Generally,No return
	if(!g_boot_part){
		extern void cmd_yaffs_rm(const char *path);
		cmd_yaffs_rm("/nand/version.bin");
		reset_cpu(0);
	}else{
	//	Still can not bootup?!
		printf("please update your system!\n");
	}
...
U_BOOT_CMD(
#ifndef CONFIG_TI814X_MIN_CONFIG	
	bootm,	CONFIG_SYS_MAXARGS,	1,	final_bootm,
#else
